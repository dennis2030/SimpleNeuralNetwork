...
%                    tmp_NN(1).a = calcActivate( pretrain_input{l,i}, activateType);
...
%                    tmp_NN(1).a = calcActivate( pretrain_input{l,i}, activateType);
                ...
                cost
                tmp_NN = updateParas(tmp_NN, learning_rate, numOfImages, weight_decay);
            end
    end
    NN(i).w = tmp_NN(1).w';
    NN(i).b = tmp_NN(1).b';
...
for i=1:max_iter
    ...
    for j=1:numOfImages
        ...
        if(mod(counter, batchSize) == 0)        
            NN = updateParas(NN, learning_rate, counter, weight_decay);
            counter = 0;
        end
    end
